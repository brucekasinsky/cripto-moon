...
export function NewWalletDetailsModal({ isOpen, onClose, wallet }: WalletDetailsModalProps) {
  ...
  }, [isOpen, wallet, selectedTimeframe]);
  ...
  const fetchWalletData = async () => {
    ...
    try {
      ...
      const [chartData, userState, userFills] = await Promise.all([
        HyperliquidService.getWalletEquityHistory(wallet.address, selectedTimeframe),
        HyperliquidService.getUserState(wallet.address),
        HyperliquidService.getUserFills(wallet.address)
      ]);

      console.log('Raw chart data:', chartData);
      console.log('User state:', userState);
      console.log('User fills:', userFills);

      // Process transactions from fills
      const transactions = userFills.map((fill: any) => ({
        id: fill.oid || `${fill.time}_${fill.coin}_${fill.side}`,
      ...
      // Calculate metrics
      const currentBalance = parseFloat(userState?.marginSummary?.accountValue || '0');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[1400px] p-0 bg-[#141414] text-white border border-white/[0.02] rounded-[18px] shadow-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={walletData.chartData}>
                            ...
                            <Area
                              ...
                              type="natural"
                              fill={(entry: any) => {
                                const value = entry.totalValue;
                                return value >= 0 ? "url(#fillPnL)" : "url(#fillPnLRed)";
                              }}
                              stroke={(entry: any) => {
                                const value = entry.totalValue;
